...
with open('./inputs/day5.txt') as f:
    ...
    ventLines = [VentLine(int(n[0]), int(n[1]), int(n[2]), int(n[3])) for n in [line.strip().replace(' -> ', ',').split(',') for line in f.readlines()]]
...
def count_double_points(ventLines, countHorizontals):
    ...
    for line in ventLines:
        ...
        if line.x1 == line.x2: xmove = 0
        elif line.x1 - line.x2 > 0: xmove = -1
        else: xmove = 1

        if line.y1 == line.y2: ymove = 0
        elif line.y1 - line.y2 > 0: ymove = -1
        else: ymove = 1

        point = (line.x1, line.y1)
        while point != (line.x2, line.y2):
            if point in points: points[point] = points[point] + 1
            else: points[point] = 1
